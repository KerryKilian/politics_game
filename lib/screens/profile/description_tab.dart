...
class DescriptionTab extends StatefulWidget {
  ...
  const DescriptionTab({Key? key, required this.userData}) : super(key: key);
    ...
    return Container(
        ...
        _isEditingBio == false ? Column(
          children: [
            ...
            SizedBox(height: 5,),
            CustomText(textAlign: TextAlign.left, text: widget.userData["bio"])

          ],
        ) : Column(
          children: [
            CustomButton(text: "Fertig", onTapFunction: () async {
              await FirestoreMethods().changeBio(_bioEditingController.text, widget.userData["uid"]);
              setState(() {
                _isEditingBio = false;
              });
            }),
